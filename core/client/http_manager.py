...
class HttpManager:
    ...
    def get_aiohttp_session(self) -> ClientSession:
        """获取 ClientSession 对象，默认使用了较短的超时时间"""
        if self._aiohttp_session is None or self._aiohttp_session.closed:
            self._aiohttp_session = ClientSession(
                timeout=ClientTimeout(connect=30, total=120)
            )
        return self._aiohttp_session

    def get_curl_session(self) -> AsyncSession:
        """获取 AsyncSession 对象，默认使用了较短的超时时间"""
        if self._curl_session is None or getattr(self._curl_session, "_closed", False):
            self._curl_session = AsyncSession(timeout=120)
